#ifndef __CC2500B_H__
#define __CC2500B_H__
...
#define SPI_CLK_0 	GPIO_ResetBits(GPIOB, GPIO_Pin_3)
#define SPI_CLK_1   GPIO_SetBits(GPIOB, GPIO_Pin_3)

#define SPI_MOSI_0   GPIO_ResetBits(GPIOB, GPIO_Pin_5)
#define SPI_MOSI_1   GPIO_SetBits(GPIOB, GPIO_Pin_5)

#define SPI_CS_0    GPIO_ResetBits(GPIOA, GPIO_Pin_15)
#define SPI_CS_1    GPIO_SetBits(GPIOA, GPIO_Pin_15)

#define SPI_MISO_IN		SPI_MISO_Input()

#define RF_GDO_IN       RF_GDO_Input()

#define CC2500_DATA_LEN  0x0d   //通信中的数据长度
#define CC2500_RX_ADDR	 0x00   //接收地址
#define CC2500_TX_ADDR   0x00   //发送地址
...
// Strobe commands
#define CCxxx0_SRES_CMD30         0x30        // Reset chip.
#define CCxxx0_SFSTXON_CMD31      0x31        // Enable and calibrate frequency synthesizer (if MCSM0.FS_AUTOCAL=1).
										// If in RX/TX: Go to a wait state where only the synthesizer is
                                        // running (for quick RX / TX turnaround).
#define CCxxx0_SXOFF_CMD32        0x32        // Turn off crystal oscillator.
#define CCxxx0_SCAL_CMD33         0x33        // Calibrate frequency synthesizer and turn it off
                                        // (enables quick start).
#define CCxxx0_SRX_CMD34          0x34        // Enable RX. Perform calibration first if coming from IDLE and
                                        // MCSM0.FS_AUTOCAL=1.
#define CCxxx0_STX_CMD35          0x35        // In IDLE state: Enable TX. Perform calibration first if
                                        // MCSM0.FS_AUTOCAL=1. If in RX state and CCA is enabled:
                                        // Only go to TX if channel is clear.
#define CCxxx0_SIDLE_CMD36        0x36        // Exit RX / TX, turn off frequency synthesizer and exit
                                        // Wake-On-Radio mode if applicable.
#define CCxxx0_SAFC_CMD37         0x37        // Perform AFC adjustment of the frequency synthesizer
#define CCxxx0_SWOR_CMD38         0x38        // Start automatic RX polling sequence (Wake-on-Radio)
#define CCxxx0_SPWD_CMD39         0x39        // Enter power down mode when CSn goes high.
#define CCxxx0_SFRX_CMD3A         0x3A        // Flush the RX FIFO buffer.
#define CCxxx0_SFTX_CMD3B         0x3B        // Flush the TX FIFO buffer.
#define CCxxx0_SWORRST_CMD3C      0x3C        // Reset real time clock.
#define CCxxx0_SNOP_CMD3D         0x3D        // No operation. May be used to pad strobe commands to two
